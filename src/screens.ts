...
const TOPICS: Record<string, string[]> = {
  ...
  install: [
    ...
    '    npx @trivial-so/cli <command>     run it without installing',
    '',
    "  If `npm i -g` asks for sudo, that is npm's global prefix rather than anything we need.",
    '  Point it somewhere you own:',
    '',
    '    npm config set prefix ~/.local    # then put ~/.local/bin on your PATH',
    '',
    '  Or take the standalone single file, which updates itself:',
    '',
